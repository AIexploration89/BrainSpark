...
import { ZOOLOGIST_RANKS } from './types';
...
export function AnimalKingdom() {
  ...
  const { updateLevelProgress, zoologistStats } = useAnimalProgressStore();
  ...
  // Handlers
  const handleSelectCategory = useCallback((category: AnimalCategory) => {
  ...
  const renderContent = () => {
    switch (gameState) {
      case 'menu':
        return (
          <MainMenu
            onStart={() => setGameState('category-select')}
...
interface MainMenuProps {
  ...
  zoologistStats: {
    ...
    habitatsExplored: import('./types').Habitat[];
    categoriesMastered: import('./types').AnimalCategory[];
    totalQuestionsAnswered: number;
    totalCorrect: number;
    totalPlayTime: number;
    longestStreak: number;
    perfectRounds: number;
    favoriteCategory: import('./types').AnimalCategory | null;
    zoologistRank: import('./types').ZoologistRank;
  };
}

function MainMenu({ onStart, onBack, zoologistStats }: MainMenuProps) {
  const { getTotalStars } = useAnimalProgressStore();
  const totalStars = getTotalStars();
  const rankInfo = ZOOLOGIST_RANKS[zoologistStats.zoologistRank];

  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 relative">
      <JungleBackground intensity="high" />
      ...
      >
        {/* Animated animal icon carousel */}
        <motion.div
          className="text-8xl sm:text-9xl mb-6 inline-block relative"
          style={{
        ...
        <h1 className="text-4xl sm:text-5xl font-display font-bold text-white mb-4">
          Animal <span className="text-neon-green" style={{ textShadow: '0 0 30px rgba(0,255,136,0.5)' }}>Kingdom</span>
        </h1>
        <p className="text-text-secondary text-lg max-w-md">
          Discover amazing animals, their habitats, and fascinating facts!
        </p>
      </motion.div>

      {/* Zoologist rank badge */}
      <motion.div
        initial={{ scale: 0.9, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        transition={{ delay: 0.2 }}
        className="mb-8 px-6 py-3 bg-gradient-to-r from-neon-green/10 to-neon-purple/10 border border-white/10 rounded-2xl flex items-center gap-4 relative z-10"
      >
        <span className="text-4xl">{rankInfo.icon}</span>
        <div className="text-left">
          <p className="text-text-muted text-xs uppercase tracking-wider">Zoologist Rank</p>
      ...
      >
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          onClick={onStart}
          className="w-full py-4 px-8 rounded-xl font-display font-bold text-lg uppercase tracking-wider bg-gradient-to-r from-neon-green to-neon-cyan text-bg-primary shadow-[0_0_30px_rgba(0,255,136,0.4)] hover:shadow-[0_0_50px_rgba(0,255,136,0.6)] transition-all"
        >
          Start Safari 🐾
        </motion.button>
      ...
      >
        <h3 className="font-display font-bold text-white text-sm mb-2">Animal Categories</h3>
        <ul className="text-text-secondary text-sm space-y-1">
          <li>🦁 <strong>Mammals</strong> - Lions, elephants, dolphins & more</li>
          <li>🦅 <strong>Birds</strong> - Eagles, penguins, parrots & more</li>
          <li>🐋 <strong>Ocean Life</strong> - Sharks, octopus, whales & more</li>
          <li>🦎 <strong>Reptiles & Amphibians</strong> - Snakes, frogs & more</li>
        </ul>
      </motion.div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex flex-col p-4 sm:p-6 relative">
      ...
      <div className="flex items-center justify-between mb-4 relative z-10">
        <div className="flex items-center gap-3">
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={onPause}
            className="w-10 h-10 flex items-center justify-center rounded-full bg-bg-secondary/80 backdrop-blur-sm border border-white/10 text-text-secondary hover:text-white hover:border-neon-green/50 transition-colors"
          >
            ⏸
          </motion.button>
          <div>
            <h2 className="font-display font-bold text-white text-sm sm:text-base">{level.name}</h2>
            <p className="text-xs text-text-muted capitalize">{level.category.replace('-', ' ')}</p>
          </div>
        </div>
      </div>
      ...
      <div className="text-center text-text-muted text-sm mt-4 relative z-10">
        Press <kbd className="px-2 py-1 bg-bg-secondary rounded text-neon-green font-mono">Esc</kbd> to pause
      </div>
    </div>
  );
}
