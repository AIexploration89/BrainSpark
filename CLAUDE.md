...
- **Timer drift**: Use wall-clock `timerDeadline` not decrement; compute `Math.ceil((deadline-Date.now())/1000)` (freq:6)
- **Zustand persist**: All stores need `version: 1`; don't store intervals in state, use component `useEffect` (freq:3)
- **Stale closures in timers/rAF**: Read via `.getState()` or ref inside interval; batch rAF updates in single `set()` (freq:3)
- **Object lookup safety**: Use `hasOwnProperty.call()` for user-supplied keys; guard division with `length > 0` (freq:2)
- **Overlay a11y**: CountdownOverlay=`role="status" aria-live`; PauseOverlay=`role="dialog" aria-modal` (freq:1)
- **Test files in tsconfig**: Exclude `src/__tests__/` from `tsconfig.app.json` to avoid vitest type errors (freq:1)
- **CSS missing definitions**: `rounded-[inherit]` not `rounded-inherit`; define `animate-shimmer` keyframes in index.css (freq:2)
